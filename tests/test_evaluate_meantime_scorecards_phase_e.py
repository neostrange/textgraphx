...
@pytest.mark.unit
def test_evaluate_batch_neo4j_mode_includes_projection_determinism(tmp_path, monkeypatch):
    gold_dir = tmp_path / "gold"
    gold_dir.mkdir()
    ...
    doc_lookup = {
        "1": evaluate_meantime.parse_meantime_xml(str(gold_dir / "doc1.xml")),
        "2": evaluate_meantime.parse_meantime_xml(str(gold_dir / "doc2.xml")),
    }

    monkeypatch.setattr(
        evaluate_meantime,
        "build_document_from_neo4j",
        lambda graph, doc_id, **kwargs: doc_lookup[str(doc_id)],
    )
    monkeypatch.setattr(
        evaluate_meantime,
        "check_projection_determinism",
        lambda graph, doc_id, **kwargs: {
            "deterministic": str(doc_id) == "1",
            "runs": int(kwargs.get("runs", 2)),
    ...
    args = argparse.Namespace(
        ...
        gold_like_nominal_filter=False,
        nominal_profile_mode="all",
        projection_determinism_runs=3,
    )

    report = evaluate_meantime._evaluate_batch(args, EvaluationMapping())

    assert "projection_determinism" in report
    pd = report["projection_determinism"]
    assert pd["runs"] == 3
    assert pd["documents_checked"] == 2
    assert pd["stable_documents"] == 1
    assert pd["unstable_documents"] == 1
    assert pd["all_stable"] is False
    assert "1" in pd["by_doc"]
    assert "2" in pd["by_doc"]
