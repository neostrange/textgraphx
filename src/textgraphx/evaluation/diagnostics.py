...
def get_registered_diagnostics() -> List[Dict[str, Any]]:
    """Return registry metadata for API or CLI discovery endpoints."""
    items = []
...
def query_phase_execution_summary(graph: Any) -> List[Dict[str, Any]]:
    """Return phase-level execution_time or duration and volume metrics."""
...
def query_entity_density(graph: Any) -> List[Dict[str, Any]]:
    """Return per-document entity, event, and timex mention density snapshots."""
...
def query_numeric_value_transition_inventory(graph: Any) -> List[Dict[str, Any]]:
    """Return transitional NUMERIC or VALUE inventory counts."""
...
__all__ = [
    ...
    "query_participation_edge_migration_inventory",
    ...
    "query_timexmention_contract_inventory",
