...
- entity: micro F1=0.196 below threshold 0.75 - mark as priority optimization track.
...
| 76437 | 0.157 | entity, event, timex, relation |
| 61327 | 0.192 | entity, event, timex, relation |
| 112579 | 0.217 | entity, event, timex, relation |
| 96770 | 0.287 | entity, event, timex, relation |
...
| relation | 3 | 107 | 20 | 0.027 | 0.130 | 0.045 |
...
- spurious (107 shown):
  ...
  - predicted=('has_participant', 'event', (116,), 'entity', (104, 105, 106), (('sem_role', 'Arg1'),))
  - predicted=('has_participant', 'event', (134,), 'entity', (160, 161), (('sem_role', 'Arg0'),))
...
| Layer | TP | FP | FN | Precision | Recall | F1 |
|---|---:|---:|---:|---:|---:|---:|
| entity | 5 | 36 | 10 | 0.122 | 0.333 | 0.179 |
| event | 6 | 29 | 6 | 0.171 | 0.500 | 0.255 |
| timex | 2 | 6 | 2 | 0.250 | 0.500 | 0.333 |
| relation | 0 | 55 | 16 | 0.000 | 0.000 | 0.000 |

Suggested actions:
- entity: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- entity: low F1 (0.179) - inspect extraction and matching rules for this layer.
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
...
- spurious (54 shown):
  ...
  - predicted=('has_participant', 'event', (151,), 'entity', (13,), (('sem_role', 'Arg0'),))
...
| relation | 4 | 51 | 22 | 0.073 | 0.154 | 0.099 |
...
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: type mismatches detected - tune label/attribute mapping in mapping-config.
- relation: low F1 (0.099) - inspect extraction and matching rules for this layer.
- relation: precision is weaker than recall - investigate over-generation and filtering criteria.
- timex: precision is weaker than recall - investigate over-generation and filtering criteria.
...
- spurious (50 shown):
  - predicted=('has_participant', 'event', (104,), 'entity', (92, 93, 94, 95), (('sem_role', 'Arg0'),))
  - predicted=('has_participant', 'event', (109,), 'entity', (92, 93, 94, 95), (('sem_role', 'Arg0'),))
  - predicted=('has_participant', 'event', (163,), 'entity', (171, 172), (('sem_role', 'Arg0'),))
...
| relation | 3 | 86 | 18 | 0.034 | 0.143 | 0.055 |
...
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: type mismatches detected - tune label/attribute mapping in mapping-config.
- relation: low F1 (0.055) - inspect extraction and matching rules for this layer.
- relation: precision is weaker than recall - investigate over-generation and filtering criteria.
- relation: type mismatches detected - tune label/attribute mapping in mapping-config.
...
- spurious (85 shown):
  ...
  - predicted=('has_participant', 'event', (121,), 'entity', (109, 110, 111, 112), (('sem_role', 'Arg0'),))
...
| relation | 3 | 32 | 19 | 0.086 | 0.136 | 0.105 |
...
- spurious (32 shown):
  ...
  - predicted=('has_participant', 'event', (174,), 'entity', (181, 182), (('sem_role', 'Arg1'),))
...
| Layer | TP | FP | FN | Precision | Recall | F1 |
|---|---:|---:|---:|---:|---:|---:|
| entity | 15 | 31 | 6 | 0.326 | 0.714 | 0.448 |
| event | 5 | 19 | 15 | 0.208 | 0.250 | 0.227 |
| timex | 3 | 7 | 1 | 0.300 | 0.750 | 0.429 |
| relation | 1 | 23 | 22 | 0.042 | 0.043 | 0.043 |

Suggested actions:
- entity: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- entity: low F1 (0.448) - inspect extraction and matching rules for this layer.
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
- entity: type mismatches detected - tune label/attribute mapping in mapping-config.
