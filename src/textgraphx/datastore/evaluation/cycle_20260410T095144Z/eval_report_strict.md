...
|---|---|---:|---:|---:|---:|
| micro | strict | entity | 0.115 | 0.387 | 0.177 |
| micro | strict | event | 0.148 | 0.369 | 0.211 |
| micro | strict | timex | 0.266 | 0.548 | 0.358 |
| micro | strict | relation | 0.055 | 0.122 | 0.076 |
| micro | relaxed | entity | 0.173 | 0.585 | 0.267 |
...
| macro | strict | event | 0.152 | 0.378 | 0.210 |
| macro | strict | timex | 0.300 | 0.562 | 0.379 |
| macro | strict | relation | 0.059 | 0.121 | 0.076 |
| macro | relaxed | entity | 0.189 | 0.621 | 0.278 |
| macro | relaxed | event | 0.214 | 0.537 | 0.296 |
| macro | relaxed | timex | 0.410 | 0.778 | 0.524 |
| macro | relaxed | relation | 0.059 | 0.121 | 0.077 |

## Relation Kind Breakdown (Micro Strict)

| Relation Kind | Precision | Recall | F1 |
|---|---:|---:|---:|
| has_participant | 0.047 | 0.218 | 0.077 |
...
- Strict Event F1: 0.211
- Strict TIMEX F1: 0.358
- Strict Relation F1: 0.076
- Composite: 0.215

### Beyond-TimeML Reasoning

- Event Gain (relaxed - strict): 0.088
...
- entity: micro F1=0.177 below threshold 0.75 - mark as priority optimization track.
...
- event: micro F1=0.211 below threshold 0.75 - mark as priority optimization track.
...
| 61327 | 0.194 | entity, event, timex, relation |
| 112579 | 0.203 | entity, event, timex, relation |
| 96770 | 0.271 | entity, event, timex, relation |
| 62405 | 0.307 | entity, event, relation |
...
| relation | 3 | 39 | 20 | 0.071 | 0.130 | 0.092 |
...
- relation: low F1 (0.092) - inspect extraction and matching rules for this layer.
...
| event | 4 | 30 | 8 | 0.118 | 0.333 | 0.174 |
...
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- event: low F1 (0.174) - inspect extraction and matching rules for this layer.
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
...
- spurious (10 shown):
  ...
  - predicted=('has_participant', 'event', (141, 142, 143), 'entity', (137,), (('sem_role', 'Arg1'),))
...
| event | 8 | 50 | 13 | 0.138 | 0.381 | 0.203 |
| timex | 5 | 2 | 1 | 0.714 | 0.833 | 0.769 |
| relation | 2 | 55 | 24 | 0.035 | 0.077 | 0.048 |
...
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- event: low F1 (0.203) - inspect extraction and matching rules for this layer.
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: type mismatches detected - tune label/attribute mapping in mapping-config.
- relation: low F1 (0.048) - inspect extraction and matching rules for this layer.
- relation: precision is weaker than recall - investigate over-generation and filtering criteria.
...
- spurious (10 shown):
  - predicted=('has_participant', 'event', (109,), 'entity', (98, 99), (('sem_role', 'Arg1'),))
...
| Layer | TP | FP | FN | Precision | Recall | F1 |
|---|---:|---:|---:|---:|---:|---:|
| entity | 5 | 87 | 12 | 0.054 | 0.294 | 0.092 |
| event | 9 | 46 | 8 | 0.164 | 0.529 | 0.250 |
| timex | 2 | 11 | 2 | 0.154 | 0.500 | 0.235 |
| relation | 5 | 80 | 16 | 0.059 | 0.238 | 0.094 |

Suggested actions:
- entity: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- entity: low F1 (0.092) - inspect extraction and matching rules for this layer.
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
...
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: type mismatches detected - tune label/attribute mapping in mapping-config.
- relation: low F1 (0.094) - inspect extraction and matching rules for this layer.
- relation: precision is weaker than recall - investigate over-generation and filtering criteria.
...
| Layer | TP | FP | FN | Precision | Recall | F1 |
|---|---:|---:|---:|---:|---:|---:|
| entity | 4 | 57 | 5 | 0.066 | 0.444 | 0.114 |
| event | 7 | 50 | 8 | 0.123 | 0.467 | 0.194 |
| timex | 2 | 10 | 5 | 0.167 | 0.286 | 0.211 |
| relation | 1 | 48 | 21 | 0.020 | 0.045 | 0.028 |

Suggested actions:
- entity: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- entity: low F1 (0.114) - inspect extraction and matching rules for this layer.
- entity: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
- event: low F1 (0.194) - inspect extraction and matching rules for this layer.
- event: precision is weaker than recall - investigate over-generation and filtering criteria.
- event: type mismatches detected - tune label/attribute mapping in mapping-config.
- relation: low F1 (0.028) - inspect extraction and matching rules for this layer.
- relation: precision is weaker than recall - investigate over-generation and filtering criteria.
- timex: boundary mismatches detected - verify token span anchoring (`start_tok`, `end_tok`) and overlap threshold.
...
- spurious (10 shown):
  - predicted={'kind': 'event', 'span': [41, 42], 'attrs': {'certainty': 'CERTAIN', 'polarity': 'POS', 'pos': 'NOUN', 'pred': 'market', 'time': 'NON_FUTURE'}}
...
- spurious (10 shown):
  - predicted={'kind': 'event', 'span': [118, 119, 120], 'attrs': {'certainty': 'CERTAIN', 'polarity': 'POS', 'pos': 'NOUN', 'pred': 'market', 'time': 'NON_FUTURE'}}
