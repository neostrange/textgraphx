...
def main():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=str(Path(__file__).resolve().parent.parent / "datastore" / "dataset"),
    ...
    if args.check:
        all_passed, messages = run_health_checks(
            str(dataset_path),
            args.model,
    ...
    all_passed, check_messages = run_health_checks(
        str(dataset_path),
        args.model,
        neo4j_uri=_neo4j_uri,
        neo4j_user=_neo4j_user,
    ...
    try:
        ...
        print(f"\n❌ Error initializing pipeline: {exc}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
    ...
    if args.preflight_safety:
        ...
        try:
            ...
            print(f"\n❌ Error during safety preflight: {exc}\n")
            sys.exit(1)
