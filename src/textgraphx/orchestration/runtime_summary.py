...
class ExecutionSummary:
    ...
    def start_phase(self, phase_name: str) -> None:
        ...
        self.phases[phase_name] = PhaseMetrics(name=phase_name, start_time=time.perf_counter())
    ...
    def success_count(self) -> int:
        ...
        return sum(1 for phase in self.phases.values() if phase.status == "completed")

    @property
    def failed_count(self) -> int:
        """Number of failed phases."""
        return sum(1 for phase in self.phases.values() if phase.status == "failed")
