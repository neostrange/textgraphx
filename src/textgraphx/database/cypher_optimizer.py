"""Canonical Cypher query optimization helpers for the database package."""
    ...
    def create_index_statements() -> List[str]:
        ...
        for node_label, indexes in CypherOptimizer.RECOMMENDED_INDEXES.items():
            for prop_name, _ in indexes:
                ...
                statements.append(
                    f"CREATE INDEX {idx_name} FOR (n:{node_label}) ON (n.{prop_name})"
                )
        return statements
...
class CypherPatternOptimizer:
    ...
    def optimize_aggregation_queries() -> Dict[str, str]:
        """Optimization patterns for metrics or aggregation queries."""
        return {
...
class QueryPerformanceContract:
    ...
    def validate_query_performance(
        ...
        if actual_duration_ms > max_duration_ms:
            return (
                False,
                f"Query '{query_name}' took {actual_duration_ms}ms, exceeds limit of {max_duration_ms}ms",
            )
        return True, None
...
def suggest_optimization_for_phase(phase_name: str) -> Dict[str, Any]:
    ...
    if phase_name == "EventEnrichmentPhase":
        return {
